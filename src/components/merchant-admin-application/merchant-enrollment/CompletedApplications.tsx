...
import { getCompletedOnboardingSessions } from "@/lib/api";
...
export default function CompletedApplications() {
  ...
  const [counts, setCounts] = useState({ approved: 0, declined: 0 });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await getCompletedOnboardingSessions();
        ...
        if (alive) {
          ...
          setCounts({
            ...
            declined: response.declined_count,
          });
        }
      } catch (error) {
      ...
      <div className="bg-jvc-blue-950 flex-1 pt-16 sm:pt-20 md:pt-24">
        ...
        <section className="bg-jvc-blue-950 text-white py-10 sm:py-16 md:py-20 px-4 md:px-16">
          <div className="max-w-7xl mx-auto">
            ...
            <h4 className="text-xl sm:text-2xl md:text-4xl font-bold leading-tight">
        ...
        <div className="bg-white">
          <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-10">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold text-[#043C6B] mb-4 sm:mb-6 md:py-4">
              Completed Merchant Enrollments
            </h2>

            {/* KPI cards (Approved / Declined) */}
            ...
            {/* <div className="flex items-center justify-between gap-4 mb-4">
              <div className="relative w-full max-w-sm">
                <input
                  ...
                  placeholder="Search by ID, merchant, state, or status…"
...
function StatusPill({ status = "" }) {
  ...
  const styleApproved = "bg-green-50 text-green-700 border-green-200";
  const styleDeclined = "bg-red-50 text-red-700 border-red-200";
  const styleDefault = "bg-gray-50 text-gray-700 border-gray-200";
  const cls = s === "approved" ? styleApproved : s === "declined" ? styleDeclined : styleDefault;
  return (
